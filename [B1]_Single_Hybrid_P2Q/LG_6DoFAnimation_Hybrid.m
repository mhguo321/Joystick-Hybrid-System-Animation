function fig = LG_6DoFAnimation_Hybrid( varargin )
...
ShowArrowHead = true;
ShowLegend = false;
Title = 'Quadcopter carrying a payload--';
Position = [50 50 866 600];
Spin = 120;
View = [(120:(Spin/(length(pQ_total)-1)):(120+Spin))', 25*ones(length(pQ_total), 1)];
% View = [(20:(Spin/(length(pQ_total)-1)):(20+Spin))', 20*ones(length(pQ_total), 1)];
AxisLength = 0.6;
LimitRatio = 1;
Xlabel = 'X(m)';
Ylabel = 'Y(m)';
...
%%
    for i = 1:numPlotSamples
        ...
        xr4 = r0*cos(temp')*eb1 + r0*sin(temp')*eb2 + ones(size(temp'))*x4;
        ratio = 0.4;
        %set(orgHandle, 'xdata', xQ, 'ydata', yQ, 'zdata', zQ,'Color','r','LineWidth',0.1);% 原点的句柄设置
